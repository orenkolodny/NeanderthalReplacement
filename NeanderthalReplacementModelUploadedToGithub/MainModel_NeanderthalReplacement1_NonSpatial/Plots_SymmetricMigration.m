...
here = '/Users/orenkolodny/Documents/workspace/NeanderthalReplacement1/output_SymmetricMigration_take2';
cd (here)
d = dir ('*Final*.txt');
mig1Probs = []; mig1sterrs =[]; mig2Probs = []; mig2sterrs =[]; mig3Probs = []; mig3sterrs =[];
d1Sizes =[];
for i = 1:length(d)
    currf = d(i).name
    currmigrate = regexprep(currf,'.+m2',''); currmigrate = regexprep(currmigrate,'\.txt',''); currmigrate = str2num(currmigrate)
    ...
    if (currmigrate==0.0005) 
        mig1Probs = [mig1Probs p];
        mig1sterrs = [mig1sterrs serr];
    elseif (currmigrate==0.0001)
        d1Sizes = [d1Sizes d1size];
        mig2Probs = [mig2Probs p];
        mig2sterrs = [mig2sterrs serr];
    elseif (currmigrate==0.00005)
        mig3Probs = [mig3Probs p];
        mig3sterrs = [mig3sterrs serr];
    end
end
% plot(popsizes, meanest,'.')
% subplot(1,2,1)
% errorbar(d1Sizes./500,mig1Probs,mig1sterrs,'.'); hold on;
errorbar(d1Sizes./500,mig2Probs,2*mig2sterrs,'b.'); hold on;
plot(d1Sizes./500,mig2Probs,'.','MarkerSize',25)
